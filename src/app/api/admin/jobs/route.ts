...
import { createJob, seedJobRecommendations } from "@/lib/platform/repository";
...
export async function POST(request:Request){const session=await getAdminSession();if(!session)return NextResponse.json({error:"unauthorized"},{status:401});const result=Schema.safeParse(await request.json());if(!result.success)return NextResponse.json({error:"invalid",issues:result.error.issues},{status:400});const id=await createJob(result.data);
  // Semeia o ranking determinístico na hora: a vaga já abre com gente, mesmo se a fila
  // atrasar ou a LLM falhar. É computação local + uma escrita, sem chamada externa.
  const seeded=(result.data.status??"open")==="open"?await seedJobRecommendations(id).catch(()=>0):0;
  const workflowId=(result.data.status??"open")==="open"?await enqueueJobWorkflow(id,session.administratorId):null;
  return NextResponse.json({id,workflowId,seeded},{status:201});}
