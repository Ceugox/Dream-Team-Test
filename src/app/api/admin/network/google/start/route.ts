...
import { buildGoogleAuthorizationUrl, googleOAuthConfig } from "@/lib/platform/google";
...
export async function GET(request: Request) {
  ...
  const redirectUri = process.env.GOOGLE_OAUTH_REDIRECT_URI?.trim() || `${new URL(request.url).origin}/api/admin/network/google/callback`;
  const store = await cookies();
  store.set(STATE_COOKIE, state, {
    httpOnly: true,
    secure: process.env.NODE_ENV === "production",
    sameSite: "lax",
    path: "/api/admin/network/google",
    maxAge: 10 * 60,
  });
  return NextResponse.redirect(buildGoogleAuthorizationUrl({ clientId: config.clientId, redirectUri, state }));
}
