...
export async function POST(request:Request){
  ...
  const result=z.object({mode:z.literal("manual"),name:z.string().trim().min(2).max(160),headline:z.string().trim().max(500).optional(),linkedinUrl:z.url().max(1000).optional().or(z.literal("")),phone:z.string().min(8).max(40)}).safeParse(body);
  ...
  await addAdminNetworkContact(session.administratorId,{...result.data,phone,linkedinUrl:result.data.linkedinUrl||undefined});const jobs=await listJobs();await Promise.all(jobs.filter(job=>job.status==="open").map(job=>refreshAdminRecommendations(job.id)));return NextResponse.json({ok:true},{status:201});
}
