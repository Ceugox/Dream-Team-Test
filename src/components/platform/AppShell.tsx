...
const adminNav = [["/admin","Visão geral","⌂"],["/admin/vagas","Vagas","◇"],["/admin/rede","Rede","⌁"],["/admin/inteligencia","IA","✦"],["/admin/equipe","Equipe","◎"],["/admin/indicacoes","Indicações","↗"]] as const;
...
export function AppShell({ mode, name, children }: { mode: "admin" | "member"; name: string; children: ReactNode }) {
  ...
  return <div className="min-h-dvh bg-[var(--background)]">
    ...
    <nav aria-label="Navegação principal" className={`fixed inset-x-0 bottom-0 z-30 grid ${mode === "admin" ? "grid-cols-6" : "grid-cols-3"} border-t border-[var(--line)] bg-[#090a0e]/95 px-1 pt-2 pb-[max(.5rem,env(safe-area-inset-bottom))] backdrop-blur lg:hidden`}>{nav.map(([href,label,icon]) => <Link key={href} href={href} className="flex min-h-12 min-w-0 flex-col items-center justify-center gap-0.5 rounded-lg px-0.5 text-[9px] text-[var(--muted)] sm:px-1 sm:text-[10px]"><span aria-hidden="true" className="text-base text-[#8197dd]">{icon}</span><span className="max-w-full truncate">{label}</span></Link>)}</nav>
  </div>;
}
