...
export type CollectionStopReason = "checkpoint" | "captcha" | "rate_limit" | "aborted";
...
interface PageSignal {
  kind: "page-signal";
  stop: Exclude<CollectionStopReason, "aborted"> | null;
}
...
async function pageSignal(page: RemotePage): Promise<Exclude<CollectionStopReason, "aborted"> | null> {
  const signal = await page.evaluate<PageSignal>(() => {
    ...
    if (text.includes("too many requests") || text.includes("429") || text.includes("rate limit")) return { kind: "page-signal", stop: "rate_limit" };
...
async function inventoryDom(page: RemotePage): Promise<RawInventoryEntry[]> {
  return page.evaluate<RawInventoryEntry[]>(() => {
    ...
    return [...document.querySelectorAll('a[href*="/in/"]')].map((anchor) => {
...
async function profileDom(page: RemotePage): Promise<RawProfessionalProfile> {
  return page.evaluate<RawProfessionalProfile>(() => {
    ...
    const fields = (root: Element | null | undefined, selector: string) => [...(root?.querySelectorAll(selector) ?? [])]
    ...
    return {
      ...
      headline: clean(document.querySelector('[data-field="headline"], [aria-label="Headline"]')?.textContent),
      location: clean(document.querySelector('[data-field="location"], [aria-label="Location"]')?.textContent),
      summary: clean(section(["about", "summary"])?.querySelector("p")?.textContent),
      fallbackFields: ["headline", "location", "summary"],
      roles: parseEntries(experience, (entry) => ({
        title: clean(entry.querySelector('[data-field="title"], h3, strong')?.textContent) ?? null,
      ...
      education: parseEntries(education, (entry) => ({
        school: clean(entry.querySelector('[data-field="school"], h3, strong')?.textContent) ?? null,
        degree: clean(entry.querySelector('[data-field="degree"]')?.textContent) ?? null,
        startDate: date(entry.querySelector("time")?.getAttribute("data-start")),
        endDate: date(entry.querySelector("time")?.getAttribute("data-end")),
      })),
      skills: fields(section(["skills"]), '[data-skill], li, a'),
      certifications: parseEntries(certification, (entry) => ({
        name: clean(entry.querySelector('[data-field="name"], h3, strong')?.textContent) ?? null,
      ...
      languages: fields(section(["languages"]), '[data-language], li, a'),
      projects: parseEntries(project, (entry) => ({
        name: clean(entry.querySelector('[data-field="name"], h3, strong')?.textContent) ?? null,
        description: clean(entry.querySelector('[data-field="description"], p')?.textContent) ?? null,
      })),
      internationalExperience: fields(section(["international"]), '[data-international], li, p'),
      mutualConnections: Number(mutualText?.match(/\d+/)?.[0]) || undefined,
    };
  }, { kind: "profile-dom" });
}
...
export async function collectConnectionInventory(page: RemotePage, options: InventoryOptions = {}): Promise<CollectionResult<InventoryEntry>> {
...
export function collectProfessionalProfile(page: RemotePage, profileUrl: string, options: CollectorOptions = {}): Promise<ProfileCollectionResult> {
  return serialize(page, async () => {
    ...
    if (!target) return { status: "stopped", reason: "checkpoint" };
