...
export type NetworkCapitalInference = { score:number; educationTier:EducationTier|null; evidence:string[]; confidence:number };
...
export function inferNetworkCapital(input:{headline:string|null;profileContext:string|null}):NetworkCapitalInference{
  ...
  if(!text.trim())return {score:0,educationTier:null,evidence:[],confidence:0};
  ...
  return {score,educationTier,evidence:Array.from(new Set(evidence)),confidence};
}
